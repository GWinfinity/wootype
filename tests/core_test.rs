...
fn test_type_universe_creation() {
    ...
    assert!(
        universe.type_count() > 0,
        "Universe should have primitive types"
    );
    println!(
        "✓ TypeUniverse created with {} types",
        universe.type_count()
    );
}
...
fn test_query_engine_creation() {
    ...
    println!(
        "✓ QueryEngine created, primitive lookup: {:?}",
        result.is_some()
    );
}
...
fn test_type_fingerprint() {
    use wootype::core::types::{PrimitiveType, TypeFingerprint};
...
fn test_error_collection() {
    use wootype::validate::error::{ErrorCollection, ErrorSeverity, SoftError};

    let mut collection = ErrorCollection::new();

    collection.add_soft_error(SoftError::new("Test error").with_severity(ErrorSeverity::Warning));
