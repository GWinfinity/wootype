...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let similar_resp = client
        .find_similar_types(SimilarTypesRequest {
            session_id: session_id.clone(),
            type_id: 1,
            threshold: 0.5,
            max_results: 10,
    ...
    println!(
        "  Found {} similar types",
        similar_resp.get_ref().total_count
    );

    // Validate expression
    println!("\n🔍 Validating expression...");
    let validate_resp = client
        .validate_expression(ValidateRequest {
            session_id: session_id.clone(),
