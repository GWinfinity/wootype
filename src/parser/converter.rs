...
impl TypeConverter {
    ...
    fn convert_type_expr<'a>(
        ...
        Box<dyn std::future::Future<Output = Result<TypeId, ConvertError>> + Send + 'a>,
    > {
        Box::pin(async move {
            match expr {
                TypeExpr::Ident(name) => self.resolve_type_by_name(name).await,
                TypeExpr::Selector { pkg, name } => self.resolve_qualified_type(pkg, name).await,
                ...
                TypeExpr::Struct(fields) => self.convert_struct_type(fields).await,
                TypeExpr::Interface(elems) => self.convert_interface_type(elems).await,
    ...
    async fn convert_interface_type(
        &self,
        elems: &[InterfaceElem],
    ...
    async fn convert_fields(&self, fields: &[Field]) -> Result<Vec<FuncParam>, ConvertError> {
        ...
        for field in fields {
            ...
            if field.names.is_empty() {
                // Unnamed parameter
                params.push(FuncParam { name: None, typ });
            } else {
                for name in &field.names {
