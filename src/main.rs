...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Commands::Query { session, type_name, package, pattern } => {
            run_query(session, type_name, package, pattern).await?;
        }
        Commands::Validate { session, expr, expected_type } => {
            run_validate(session, expr, expected_type).await?;
        }
        Commands::Connect { name, agent_type, isolation } => {
...
async fn run_daemon(
    ...
    info!("Type universe initialized with {} types", universe.type_count());
