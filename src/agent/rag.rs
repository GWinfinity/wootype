...
use crate::core::{TypeId, Type, TypeKind, SharedUniverse};
use dashmap::DashMap;
use std::sync::Arc;
...
impl TypeEmbeddings {
    ...
    pub fn find_similar(&self, type_id: TypeId, limit: usize) -> Vec<SearchResult> {
        let query_vector = self.embeddings
            .get(&type_id)
...
mod tests {
    ...
    async fn test_semantic_search() {
        ...
        // Index some types
        let types = vec![
            Arc::new(Type::new(TypeId(1), TypeKind::Primitive(PrimitiveType::Int))),
            Arc::new(Type::new(TypeId(2), TypeKind::Primitive(PrimitiveType::String))),
        ];
