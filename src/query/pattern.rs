...
use crate::core::{Type, TypeKind, TypeFlags, TypeId};
use regex::Regex;
...
pub enum TypeKindPattern {
    ...
    Array { len: Option<u64>, elem: Box<TypePattern> },
    Map { key: Box<TypePattern>, value: Box<TypePattern> },
    Chan { dir: ChanDirPattern, elem: Box<TypePattern> },
    Func { params: Vec<TypePattern>, results: Vec<TypePattern> },
    Struct { fields: Vec<(String, TypePattern)> },
    Interface { methods: Vec<(String, TypePattern)> },
}
...
impl TypeKindPattern {
    pub fn matches(&self, kind: &TypeKind) -> bool {
        match (self, kind) {
            ...
            (Self::Array { len, elem }, TypeKind::Array { len: arr_len, elem: arr_elem }) => {
                len.map_or(true, |l| l == *arr_len)
            }
            (Self::Map { key, value }, TypeKind::Map { key: m_key, value: m_value }) => true,
            (Self::Chan { dir, elem }, TypeKind::Chan { dir: c_dir, elem: c_elem }) => {
                dir.matches(c_dir)
            }
            (Self::Func { params, results }, TypeKind::Func { params: f_params, results: f_results, .. }) => {
            ...
            (Self::Interface { methods }, TypeKind::Interface { methods: i_methods, .. }) => {
