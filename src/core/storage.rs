...
impl<T: Send + Sync + 'static> DenseStorage<T> {
    ...
    pub fn get(&self, entity: EntityId) -> Option<&T> {
        self.entity_to_index.get(&entity).map(|&i| &self.components[i])
    }
    
    pub fn get_mut(&mut self, entity: EntityId) -> Option<&mut T> {
        self.entity_to_index.get(&entity).copied().map(|i| &mut self.components[i])
    }
...
impl Archetype {
    ...
    pub fn get_component_mut<T: Send + Sync + 'static>(&mut self, entity_id: EntityId) -> Option<&mut T> {
        let type_id = StdTypeId::of::<T>();
...
impl<T: Send + Sync + 'static> ArchetypeStorage<T> {
    ...
    pub fn get_or_create(&self, id: ArchetypeId) -> dashmap::mapref::one::RefMut<ArchetypeId, Archetype> {
        self.archetypes.entry(id).or_insert_with(|| Archetype::new(id))
    }
    ...
    pub fn query<C: Send + Sync + 'static, F, R>(&self, archetype_id: ArchetypeId, f: F) -> Option<R>
    where
        F: FnOnce(&DenseStorage<C>) -> R,
    {
        self.archetypes.get(&archetype_id).and_then(|a| {
            let type_id = StdTypeId::of::<C>();
            a.storages.get(&type_id)
    ...
    pub fn iter(&self) -> impl Iterator<Item = dashmap::mapref::multiple::RefMulti<'_, ArchetypeId, Archetype>> {
